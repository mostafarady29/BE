...
export default function UsersPage() {
  ...
  return (
    <div className="p-8 max-w-300 mx-auto min-h-screen" dir="rtl">
      {/* Header */}
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-text-secondary">
            إضافة، تعديل وإيقاف حسابات الموظفين
          </p>
        </div>
        <button
          onClick={() => openModal()}
                ...
                users.map((u) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-2 py-1 rounded text-xs font-semibold ${u.role === "admin" ? "bg-purple-100 text-purple-800" : "bg-blue-100 text-blue-800"}`}
                      >
                        {u.role === "admin" ? "مدير" : "موظف"}
                      </span>
                    </td>
                    <td
                      className="px-6 py-4 text-text-secondary"
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in">
            <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-gray-50/50">
              <h3 className="font-bold text-navy text-lg">
                {editId ? `تعديل: ${username}` : "إضافة موظف جديد"}
              </h3>
              <button
                onClick={closeModal}
