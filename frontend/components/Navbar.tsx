  ...
  return (
    <nav className="sticky top-0 z-100 bg-white border-b border-border shadow-[0_1px_12px_rgba(30,111,168,0.07)]">
      <div className="max-w-container mx-auto px-4 sm:px-8 flex items-center justify-between h-17.5">
        ...
        {/* Mobile menu button */}
        <button
          className="md:hidden flex flex-col justify-center items-center gap-[4px] w-9 h-9 rounded-md bg-blue-faint hover:bg-gray-100 transition-colors"
          onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
          aria-label="القائمة"
        >
          <span className={`h-0.5 w-5 bg-blue transition-transform duration-300 ${isMobileMenuOpen ? 'rotate-45 translate-y-[6px]' : ''}`} />
          <span className={`h-0.5 w-5 bg-blue transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-0' : 'opacity-100'}`} />
          <span className={`h-0.5 w-5 bg-blue transition-transform duration-300 ${isMobileMenuOpen ? '-rotate-45 -translate-y-[6px]' : ''}`} />
        </button>
        ...
        <div className="hidden md:flex items-center gap-1">
          {links.map((link, i) => (
            ...
            >
              {link.label}
              <span
                className={`nav-underline transition-transform duration-250 ease-in-out ${hovered === i ? "scale-x-100" : "scale-x-0"
                  }`}
              />
            </Link>
          ))}
      ...
      {/* Links - Mobile */}
      <div
        className={`md:hidden absolute top-full left-0 w-full bg-white border-b border-border shadow-blue-md overflow-hidden transition-all duration-300 ease-in-out ${isMobileMenuOpen ? "max-h-96 border-t border-border-light" : "max-h-0 border-transparent border-t-0"
          }`}
      >
